  ...
  "
            ...
            <div className="flex-1 flex items-center gap-6 overflow-hidden">
                ...
                {linkedPage ? (
                    <div className="flex items-center gap-3 bg-blue-50/50 p-2 pr-4 rounded-2xl border border-blue-100">
                        ...
                        <img src={`${baseUrl}${linkedPage.imageUrl}`} className="w-14 h-14 object-cover rounded-lg border-2 border-white shadow-sm" />
                    </div>
                ) : (
            ...
            <div className="flex gap-2">
                ...
                <div className="tooltip tooltip-bottom" data-tip="เชื่อมลิงก์เมนู">
                    <button
                        className={`btn btn-circle btn-sm ${linkedPage
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center bg-stone-900/80 backdrop-blur-md p-6 animate-in fade-in">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-5xl h-[85vh] flex overflow-hidden shadow-2xl">
                        <div className="w-2/5 bg-stone-50 p-10 flex flex-col items-center justify-center border-r">
                            <img src={`${baseUrl}${menu.imageUrl}`} className="w-full rounded-2xl shadow-xl border-4 border-white mb-6" />
                            <div className="badge badge-primary font-bold">ต้นทาง</div>
                        </div>
                        <div className="flex-1 flex flex-col">
                            <div className="p-6 border-b flex justify-between items-center bg-white">
                                <h3 className="font-black text-xl">เลือกรูปปลายทาง</h3>
                                <button onClick={() => setIsModalOpen(false)} className="btn btn-ghost btn-circle">✕</button>
                            </div>
                            <div className="flex w-full justify-center p-4">
                                <button onClick={() => { onUpdateCategory(menu.id, null); setIsModalOpen(false); }} className="btn btn-outline btn-block rounded-xl">ยกเลิกการเชื่อมลิงก์</button>
                            </div>
                            <div className="grid grid-cols-2 overflow-y-auto p-6 space-y-3 bg-stone-50/50">
                                {pages.map((p, index) => (
                                    <div
                                        key={p.id}
                                        onClick={() => { onUpdateCategory(menu.id, p.id); setIsModalOpen(false); }}
    ...
    text-white text-xs font-black flex items-center justify-center shadow-md`}
                                        ...
                                        <img src={`${baseUrl}${p.imageUrl}`} className="w-40 h-full object-cover rounded-xl" />
                                        {
                                            menu.category == p.id && (
                                                <span className="absolute top-3 right-3 z-10 w-7 h-7 rounded-full bg-primary text-white text-xs font-black flex items-center justify-center shadow-md">
